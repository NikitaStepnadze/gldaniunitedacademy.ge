...
/**
 * Fills the `<!--cms:events-->` placeholder with the cards an admin has
 * created.
 *
 * The home page's events section used to carry two cards written into the
 * theme HTML by hand, so editing one meant editing the file. The markup now
 * carries a placeholder instead, and the cards come from the events table --
 * which is what lets an admin add a third entry, reorder them, or choose which
 * two appear here, without anything being redeployed.
 *
 * An empty list removes the whole section rather than leaving a heading over a
 * blank strip: an academy with nothing scheduled should show no schedule, not
 * an empty one. The section is identified by the comment the theme wraps it in,
 * which is the one landmark in that markup that is unambiguous -- the wrapper
 * divs are not distinguishable from their neighbours by class alone.
 */
/**
 * Swaps the generated programme cards into the home page's programmes section.
 *
 * The section's three cards used to be written out in the template, which is
 * why every field of them had its own CMS marker. They come from the programmes
 * table now, so the template carries a placeholder instead and this puts the
 * rendered cards where they were.
 *
 * A page with no placeholder is returned untouched, so this is safe to call for
 * every route rather than only the one that has it.
 */
export function applyPrograms(markup, cardsHtml) {
  const PLACEHOLDER = '<!--cms:programs-->';
  if (!markup.includes(PLACEHOLDER)) return markup;

  if (cardsHtml.trim() !== '') return markup.replace(PLACEHOLDER, cardsHtml);

  /*
   * Nothing to show: drop the whole section rather than leave its heading over
   * an empty row. The span is matched from the widget's opening tag to the
   * comment that closes it, both of which the theme writes exactly once, and
   * the match is non-greedy so it cannot run past the first closing marker.
   */
  return markup.replace(
    /<div class="tf-widget-course main-content" id="programs">[\s\S]*?<!-- Widget Course -->/,
    ''
  );
}

/**
 * Fills the programme detail template with one programme.
 *
 * Two placeholders rather than one: the title appears in the page heading and
 * again in the breadcrumb trail, and both are plain text positions, so the
 * value is escaped here rather than trusted from the caller. The detail block
 * is markup the renderer has already escaped field by field, so it is
 * substituted as-is.
 */
export function applyProgramDetail(markup, { title, detailHtml }) {
  return markup
    .replaceAll('<!--cms:program-title-->', escapeHtml(String(title ?? '')))
    .replace('<!--cms:program-detail-->', detailHtml ?? '');
}

export function applyEvents(markup, cardsHtml) {
  const PLACEHOLDER = '<!--cms:events-->';
  if (!markup.includes(PLACEHOLDER)) return markup;
   ...
   *
   * The span is matched from the widget's opening tag to the comment that
   * closes it, both of which the theme writes exactly once. A non-greedy match
   * cannot run past the first closing marker, so this cannot swallow a later
   * section even if the opening tag were ever duplicated.
   */
  return markup.replace(
    /<div class="tf-widget-event main-content-medium">[\s\S]*?<!-- Widget event -->/,
    ''
  );
}

/**
 * Fills the `<!--cms:news-->` placeholder on /news with the blog cards.
 *
 * Mirrors applyEvents, and for the same reason: the four posts in this list
 * were hardcoded articles in the home page's blog section, so adding a fifth
 * meant editing the template. They are rows now.
 *
 * An empty list removes the section rather than leaving a heading over a blank
 * row -- an academy that has published nothing should show no news list.
 */
export function applyNews(markup, cardsHtml) {
  const PLACEHOLDER = '<!--cms:news-->';
  if (!markup.includes(PLACEHOLDER)) return markup;

  if (cardsHtml.trim() !== '') return markup.replace(PLACEHOLDER, cardsHtml);

  return markup.replace(
    /<!-- widge blog -->[\s\S]*?<\/div><!-- widge blog -->/,
    ''
  );
}

/**
 * Fills the `<!--cms:events-all-->` placeholder on /news.
 *
 * Separate from applyEvents because the two lists differ in what they hold:
 * the home page shows the two entries an admin has flagged, this one shows
 * every published schedule entry. Sharing a placeholder would make it
 * impossible for one page to render a different set from the other.
 */
export function applyEventsAll(markup, cardsHtml) {
  const PLACEHOLDER = '<!--cms:events-all-->';
  if (!markup.includes(PLACEHOLDER)) return markup;
