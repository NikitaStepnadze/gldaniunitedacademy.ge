...
const LIMITS = {
  ...
  schoolTo: 8,
  idNumber: 32,
  // Holds a plan key ('standard' / 'extended'), not the Georgian label, so the
  // column stays small; see TRAINING_PLANS.
  trainingPlan: 32,
};
...
/**
 * The training plans a parent chooses between when applying.
 *
 * Rows store the key ('standard' / 'extended'), never the Georgian sentence.
 * The price and the wording are the sort of thing an academy revises between
 * seasons, and storing the label would freeze one season's copy into every
 * application ever submitted -- so past rows would keep asserting a price that
 * is no longer offered. Keeping the label here means changing it re-renders
 * every row, in the admin panel and in the export alike.
 *
 * `price` is null for the extended plan on purpose: it is settled in person,
 * and inventing a number to fill the column would be worse than saying so.
 */
export const TRAINING_PLANS = {
  standard: {
    label: '130₾/თვეში, კვირაში 3 ვარჯიში',
    short: '130₾ — 3 ვარჯიში',
    price: 130,
  },
  extended: {
    label: 'კვირაში 3 ვარჯიში + 2 დამატებითი ვარჯიში (ფასი ადგილზე განხილვით)',
    short: '3 + 2 ვარჯიში',
    price: null,
  },
};

/** The valid keys, for validating a submission and filtering the admin list. */
export const TRAINING_PLAN_KEYS = Object.keys(TRAINING_PLANS);

/** The label for a stored plan key, or '—' when a row carries none. */
export function trainingPlanLabel(value) {
  return TRAINING_PLANS[value]?.label ?? '—';
}
...
export function validateRegistration(input, now = new Date()) {
  ...
  const trainingPlan = clean(input?.trainingPlan, LIMITS.trainingPlan);
  ...
  /*
   * Checked against the known keys, not merely for being non-empty.
   *
   * The value comes from a radio group, so anything other than one of these
   * two keys was not sent by the form -- and storing it would put a plan in
   * the row that the admin panel cannot render and the academy does not offer.
   */
  if (!trainingPlan) errors.trainingPlan = 'required';
  else if (!TRAINING_PLAN_KEYS.includes(trainingPlan)) {
    errors.trainingPlan = 'invalid';
  }
...
function toPlainEnquiry(row) {
  return {
    ...
    // Empty on contact enquiries and on registrations submitted before the
    // choice existed; the admin panel renders those as '—'.
...
function haystackOf(row) {
  return [
    ...
    row.message,
    row.notes,
    // The plan's label, not its key: an admin searching for it types "130" or
    // "დამატებითი", never "standard".
    TRAINING_PLANS[row.trainingPlan]?.label,
  ]
    .filter(Boolean)
    .join(' ')
    .toLowerCase();
...
export async function listEnquiries({
  ...
  // Filtered here rather than in Appwrite for the same reason as `source`: the
  // column is unset on every row written before the choice existed, and an
  // equality query would silently drop those instead of simply not matching.
  if (plan) rows = rows.filter((row) => row.trainingPlan === plan);

  // Applications nobody has opened yet -- the same flag that tints a row in
  // the admin table, exposed as a filter so the office can pull just the
  // backlog instead of scanning the full list for the coloured rows.
  if (unseen) rows = rows.filter((row) => !row.seen);

  /*
   * Registration date range, inclusive on both ends. `dateTo` is bumped to the
   * end of that calendar day rather than compared as midnight -- otherwise
   * picking the same day for both ends would match nothing, since every row
   * created that day has a time-of-day after 00:00.
   */
  if (dateFrom) {
...
export function validateEnquiryEdit(input, { isRegistration } = {}) {
  ...
  if (isRegistration) {
    ...
    /*
     * The training plan. Blank is allowed here but not on the public form:
     * applications submitted before the choice existed have no plan, and an
     * admin editing such a row's address must not be forced to invent one.
     * A value that is present still has to be a plan we offer.
     */
    const plan = text('trainingPlan', LIMITS.trainingPlan);
    if (plan && !TRAINING_PLAN_KEYS.includes(plan)) errors.trainingPlan = 'invalid';
