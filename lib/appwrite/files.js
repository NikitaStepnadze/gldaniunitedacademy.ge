...
/**
 * Image types accepted for the child's photo.
 *
 * A subset of ALLOWED_TYPES: a PDF is a valid document but not a portrait, and
 * the admin panel renders this one inline as an `<img>`.
 */
const PHOTO_TYPES = new Set(['image/jpeg', 'image/png', 'image/webp', 'image/heic']);

/**
 * Uploads the child's photo, returning its id.
 *
 * Kept separate from uploadEnquiryFiles so the type rule can be narrower and so
 * the resulting id can be stored in its own column.
 */
export async function uploadChildPhoto(file) {
  if (!file || typeof file.arrayBuffer !== 'function' || file.size === 0) {
    throw new Error('photo_required');
  }
  if (!PHOTO_TYPES.has(file.type)) throw new Error('photo_type_not_allowed');
  return uploadEnquiryFile(file);
}
...
/** Exposed so the registration route can describe what it accepts. */
export const photoTypes = [...PHOTO_TYPES];
