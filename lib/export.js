...
import { STATUS_LABELS, trainingPlanLabel } from './appwrite/enquiries';
...
export const FIELDS = [
  ...
  // Rendered from the stored key, so a change of price or wording reprints
  // every past application under the new label rather than the old one.
  {
    header: 'ვარჯიშის გეგმა',
    width: 44,
    registrationOnly: true,
    value: (r) => trainingPlanLabel(r.trainingPlan),
  },
 ...
 *
 * Stored as plain `YYYY-MM-DD` (see DOB_PATTERN in enquiries.js), which is
 * unambiguous in the database but reads oddly in a Georgian office's
 * spreadsheet -- so it is rewritten into the same day-first form the rest of
 * the workbook uses, rather than exported as-is.
 */
